...
// key can take value 1 to 6 /////////////////// KEY A PROTEGER
void	shift_gradient(t_global *s_glb, int key)
{
	...
	s_glb->s_fract->gradient = line_grad(begin, end, s_glb->s_fract->max_iter);
	if (close(fd) < 0)
	{
		perror("Close failed");
		free_global_ex(s_glb, EXIT_FAILURE);
...
// Frees the current init struc of s_glb
// Assigns a new one, shifted depending on key arg
void	shift_init_point(t_global *s_glb, int key)
{
	double	r;
	double	i;

	r = s_glb->s_fract->s_init->r * 1000;
	i = s_glb->s_fract->s_init->i * 1000;
	r = (long) r;
	i = (long) i;
	...
	if (key == 119)
		...
		r = r + 10;
	s_glb->s_fract->s_init = new_cpx_pt(fma(r, 0.001, 0), fma(i, 0.001, 0));
