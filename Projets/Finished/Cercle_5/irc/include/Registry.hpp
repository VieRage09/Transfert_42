...
public:
  ...
  // Norme coplienne
  Registry(const Registry& other);                    // Constructeur de copie
  Registry& operator=(const Registry& other);         // Opérateur d'assignation
  ~Registry();                                        // Destructeur
  ...
  // +t / -t (protection du topic)
  bool          setChannelMode_topic(Channel* ch, bool add,
                                    Client* actor, std::string& err, TargetSet& notify);

  // +i / -i (invite-only)
  bool          setChannelMode_invite(Channel* ch, bool add,
                                     Client* actor, std::string& err, TargetSet& notify);
