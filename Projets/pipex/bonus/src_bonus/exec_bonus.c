...
/*   Updated: 2024/06/05 18:09:59 by tlebon           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void middle_cmd_loop(char **cmd_tab, int **pipe_tab, char **env)
{
	...
	i = 1;
	while (cmd_tab[i] && id != 0)// The child process is not supposed to loop at any time
	{
		id = fork();
		if (id == -1)
		{
		...
		if (id == 0)
		{
			if (redirect(pipe_tab[i], pipe_tab[i - 1][0],pipe_tab[i][1]) == -1)
				exit(4);
			exit(execute(cmd_tab[i], env));
		}
		i++;
	}
	return ;
