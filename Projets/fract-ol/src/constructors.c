...
/*   Updated: 2024/02/22 03:01:08 by tlebon           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	mlx_data_struct_constructor(t_mlx_data *s_mlx, t_reso *s_reso, char *win_name)
{
	...
	if(!s_mlx->mlx)
		return (1);
	s_mlx->win = mlx_new_window(s_mlx->mlx, s_reso->x_reso, s_reso->y_reso, win_name);
	if (!s_mlx->win)
		return (1);
	return (0);
}
...
int	img_data_struct_constructor(t_mlx_data *s_mlx, t_img_data *s_img, t_reso *s_reso)
{
	...
	if (!s_img->img)
		return (1);
	s_img->addr = mlx_get_data_addr(s_img->img, &(s_img->bit_per_pixel), &(s_img->line_length), &(s_img->endian));
	// printf("Image created : \nbpp = %i\nline_length = %i\nendian = %i\n", s_img->bit_per_pixel, s_img->line_length, s_img->endian);
	return (0);
}
